...
def train_test_split(X_transformed, y, stimulus_numbers1, stimulus_numbers2, test_stimulus1, test_stimulus2):
    ...
    X_train = X_transformed[train_mask]  
    Y_train = y[train_mask]  
    X_test = X_transformed[test_mask]    
    Y_test = y[test_mask]   
...
def evaluate_over_time(data_hue1, data_hue2, time_bins, stim_numbers1, stim_numbers2):
    ...
    for t in time_bins:
        ...
        time_accuracies = []
        X_reshaped, y = windowed_data_iteratively(data_hue1, data_hue2, t, window_size = 20)
        
        X_transformed = Euclidean_Distance(X_reshaped)
        
        for stim1 in np.unique(stim_numbers1):
            for stim2 in np.unique(stim_numbers2):
                if stim1 == stim2: 
                    X_train, Y_train, X_test, Y_test = train_test_split(X_transformed, y, stim_numbers1, stim_numbers2, stim1, stim2)
...
def process_participant(participant, data_dir="data/experiment_hue"):
    ...
    for pair_key, acc_list in mean_accuracies.items():
        ...
        np.savetxt(os.path.join(output_dir, f"{participant}_w20_Euclidean_Distance_mean_{pair_key}.csv"), mean_acc, delimiter=",")
    ...
    np.savetxt(os.path.join(output_dir, f"{participant}_w20_Euclidean_Distance_mean.csv"), mean_hue_accuracy, delimiter=",")
...
if __name__ == "__main__":
    ...
    os.makedirs(output_dir, exist_ok=True)
    np.savetxt(os.path.join(output_dir, "Overall_w20_Euclidean_Distance_mean.csv"), overall_mean_hue_accuracy, delimiter=",")
    np.savetxt(os.path.join(output_dir, "Overall_w20_Euclidean_Distance_ci.csv"), ci, delimiter=",")
    ...
    plt.xlabel('Time (ms)')
    plt.ylabel('Accuracy')
    plt.title('Average Classifier Accuracy Over Time - Euclidean Distance (w=20ms)')
    ...
    plt.ylim(0.45, 0.7)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "Overall_w20_Euclidean_Distance.png"))
    plt.show()
