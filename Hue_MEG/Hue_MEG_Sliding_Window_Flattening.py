...
def evaluate_over_time(data_hue1, data_hue2, time_bins, stim_numbers1, stim_numbers2):
    ...
    for t in time_bins:
        ...
        time_accuracies = []
        X_pca, y = windowed_data_iteratively(data_hue1, data_hue2, t, window_size = 20)
        
        for stim1 in np.unique(stim_numbers1):
            for stim2 in np.unique(stim_numbers2):
                if stim1 == stim2:  
                    X_train, Y_train, X_test, Y_test = train_test_split(X_pca, y, stim_numbers1, stim_numbers2, stim1, stim2)
...
def process_participant(participant, data_dir="data/experiment_hue"):
    ...
    for pair_key, acc_list in mean_accuracies.items():
        ...
        np.savetxt(os.path.join(output_dir, f"{participant}_w20_Sliding_Window_Flattening_mean_{pair_key}.csv"), mean_acc, delimiter=",")
    ...
    np.savetxt(os.path.join(output_dir, f"{participant}_w20_Sliding_Window_Flattening_mean.csv"), mean_hue_accuracy, delimiter=",")
...
if __name__ == "__main__":
    ...
    os.makedirs(output_dir, exist_ok=True)
    np.savetxt(os.path.join(output_dir, "Overall_w20_Sliding_Window_Flattening_mean.csv"), overall_mean_hue_accuracy, delimiter=",")
    np.savetxt(os.path.join(output_dir, "Overall_w20_Sliding_Window_Flattening_ci.csv"), ci, delimiter=",")
    ...
    plt.ylim(0.45, 0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "Overall_w20_Sliding_Window_Flattening.png"))
    plt.show()
