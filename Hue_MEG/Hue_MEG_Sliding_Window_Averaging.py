...
def process_participant(participant):
    ...
    for LumOffset in range(1, 4):
        ...
        for i in range(len(hues)):
            for j in range(i + 1, len(hues)):
                ...
                # Get stimulus numbers (1-based indexing)
    ...
    for pair_key, acc_list in mean_accuracies.items():
        ...
        np.savetxt(os.path.join(data_path, f"{participant}_w20_Sliding_Window_Averaging_mean{pair_key}.csv"), mean_acc, delimiter=",")
    ...
    np.savetxt(os.path.join(data_path, f"{participant}_w20_Sliding_Window_Averaging_mean.csv"), mean_hue_accuracy, delimiter=",")
...
if __name__ == "__main__":
    ...
    np.savetxt("/data/z5452142/experiment_hue/Overal_w20_Sliding_Window_Averaging_mean.csv", overall_mean_hue_accuracy, delimiter=",")
    np.savetxt("/data/z5452142/experiment_hue/Overal_w20_Sliding_Window_Averaging_ci.csv", ci, delimiter=",")
    ...
    plt.xlabel('Time (ms)')
    plt.ylabel('Accuracy')
    plt.title('Average Classifier Accuracy Over Time - Sliding_Window_Averaging  w=20ms')
    ...
    plt.tight_layout()
    plt.savefig("Overall_w20_Sliding_Window_Averaging.png")
    plt.show()
