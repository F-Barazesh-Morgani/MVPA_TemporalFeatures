...
def windowed_data_iteratively(data_hue1, data_hue2, time_bin, window_size):
    ...
    num_trials, num_features, num_time_bins = data_hue1.shape
    ...
    data1_windowed = data_hue1[:, :, start_idx:end_idx]
    data2_windowed = data_hue2[:, :, start_idx:end_idx]
       
    # Average over the time dimension
    data1_averaged = data1_windowed.mean(axis=2)  
    data2_averaged = data2_windowed.mean(axis=2)  
       
    # Combine data from both categories
    X = np.concatenate([data1_averaged, data2_averaged], axis=0)  
...
def evaluate_over_time(data_hue1, data_hue2, time_bins, stim_numbers1, stim_numbers2):
    ...
    for t in time_bins:
        ...
        time_accuracies = []
        X_scaled, y = windowed_data_iteratively(data_hue1, data_hue2, t, window_size = 20)
...
def load_class_dat(data_path, file_name):
    with h5py.File(os.path.join(data_path, file_name), 'r') as f:
        ...
        TrialList = data_group['TrialList'][:].T  # Shape: (4065, 10)
        class_dat = data_group['class_dat'][:].transpose(2, 0, 1)  # Shape: (4065, 252, 221)
...
def process_participant(participant, data_dir="data/experiment_hue"):
    ...
    for LumOffset in range(1, 4):
        ...
        for i in range(len(hues)):
            for j in range(i + 1, len(hues)):
                ...
                accuracies = evaluate_over_time(
                    hue_data[hue_a], hue_data[hue_b], time_bins,
    ...
    for pair_key, acc_list in mean_accuracies.items():
        ...
        np.savetxt(os.path.join(output_dir, f"{participant}_w20_Sliding_Window_Averaging_mean{pair_key}.csv"), mean_acc, delimiter=",")
    ...
    np.savetxt(os.path.join(output_dir, f"{participant}_w20_Sliding_Window_Averaging_mean.csv"), mean_hue_accuracy, delimiter=",")
...
if __name__ == "__main__":
    ...
    os.makedirs(output_dir, exist_ok=True)
    np.savetxt(os.path.join(output_dir, "Overall_w20_Sliding_Window_Averaging_mean.csv"), overall_mean_hue_accuracy, delimiter=",")
    np.savetxt(os.path.join(output_dir, "Overall_w20_Sliding_Window_Averaging_ci.csv"), ci, delimiter=",")
    ...
    plt.xlabel('Time (ms)')
    plt.ylabel('Accuracy')
    plt.title('Average Classifier Accuracy Over Time - Sliding Window Averaging (w=20ms)')
    plt.legend(loc='lower right')
    plt.xticks(np.arange(-100, 1001, 100))
    plt.yticks(np.arange(0.45, 0.61, 0.05))
    plt.xlim(-100, 1000)
    plt.ylim(0.45, 0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "Overall_w20_Sliding_Window_Averaging.png"))
    plt.show()
