...
def transform(X):
    ...
    for sample in range(num_samples):
        ...
        for feature in range(num_features):
            ts = X[sample, feature, :]  
            catch22_results = pycatch22.catch22_all(ts)
            sample_features.extend(catch22_results["values"])  
        transformed_features.append(sample_features)
     
    X_transformed = np.array(transformed_features)
...
def evaluate_over_time(data_hue1, data_hue2, time_bins, stim_numbers1, stim_numbers2):
    ...
    for t in time_bins:
        time_accuracies = []
        X, y = windowed_data_iteratively(data_hue1, data_hue2, t, window_size = 20)
        X_transformed_pca = transform(X)
        
        for stim1 in np.unique(stim_numbers1):
            for stim2 in np.unique(stim_numbers2):
                if stim1 == stim2:       
                    X_train, Y_train, X_test, Y_test = train_test_split(X_transformed_pca, y, stim_numbers1, stim_numbers2, stim1, stim2)
...
def process_participant(participant, data_dir="data/experiment_hue"):
    ...
    # Save results
    output_dir = os.path.join(data_dir, participant, "results")
    os.makedirs(output_dir, exist_ok=True)

    for pair_key, acc_list in mean_accuracies.items():
        acc_array = np.array(acc_list)
        mean_acc = np.mean(acc_array, axis=0)
        np.savetxt(os.path.join(output_dir, f"{participant}_w20_Catch22_mean_{pair_key}.csv"), mean_acc, delimiter=",")
    ...
    np.savetxt(os.path.join(output_dir, f"{participant}_w20_Catch22_mean.csv"), mean_hue_accuracy, delimiter=",")
...
if __name__ == "__main__":
    ...
    output_dir = "data/experiment_hue/results"
    os.makedirs(output_dir, exist_ok=True)
    np.savetxt(os.path.join(output_dir, "Overall_w20_Catch22_mean.csv"), overall_mean_hue_accuracy, delimiter=",")
    np.savetxt(os.path.join(output_dir, "Overall_w20_Catch22_ci.csv"), ci, delimiter=",")
    ...
    plt.xlabel('Time (ms)')
    plt.ylabel('Accuracy')
    plt.title('Average Classifier Accuracy Over Time - Catch22  (w=20ms)')
    ...
    plt.tight_layout()
    plt.savefig("Overall_w20_Catch22_mean.png")
    plt.show()
