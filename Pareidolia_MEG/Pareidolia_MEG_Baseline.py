...
def windowed_data_iteratively(data1, data2, time_bin, window_size):
    ...
    data1_windowed = data1[:, :, time_bin]
    data2_windowed = data2[:, :, time_bin]
...
def evaluate_over_time(data1, data2, time_bins, stimulus_numbers1, stimulus_numbers2):
    ...
    for t in time_bins:
        time_accuracies = []
        X_pca, y = windowed_data_iteratively(data1, data2, t, window_size = 0)
...
def process_participant(participant):
    ...
    plt.savefig(f"{participant}_Baseline.png")
    plt.show()
