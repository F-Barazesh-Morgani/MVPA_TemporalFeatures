...
def process_participant(participant, base_dir="data/experiment_object"):
    ...
    for i in range(len(object_labels)):
        for j in range(i + 1, len(object_labels)):
            ...
            np.savetxt(
                os.path.join(data_path, f"{participant}_w20_Catch22_{key}.csv"),
                acc,
                delimiter=","
            )
    ...
    np.savetxt(
        os.path.join(data_path, f"{participant}_w20_Catch22_mean.csv"),
        mean_object_accuracy,
        delimiter=","
    )
...
if __name__ == "__main__":
    ...
    np.savetxt(os.path.join(BASE_DIR, "Overal_w20_Catch22_mean.csv"), overall_mean_object_accuracy, delimiter=",")
    np.savetxt(os.path.join(BASE_DIR, "Overal_w20_Catch22_ci.csv"), ci, delimiter=",")
