...
def evaluate_over_time(data_object1, data_object2, time_bins, seq_numbers1, seq_numbers2):
    ...
    for t in time_bins:
        time_accuracies = []
        X, y = windowed_data_iteratively(data_object1, data_object2, t, window_size = 20)
        X_transformed_pca = transform(X)
    
        for stim1 in np.unique(seq_numbers1):
            for stim2 in np.unique(seq_numbers2):
                if stim1 == stim2: 
                    X_train, Y_train, X_test, Y_test = train_test_split(X_transformed_pca, y, seq_numbers1, seq_numbers2, stim1, stim2)
...
def process_participant(participant):
    ...
    for i in range(len(object_labels)):
        for j in range(i + 1, len(object_labels)):
            ...
            np.savetxt(os.path.join(data_path, f"{participant}_w20_Catch22_{key}.csv"), acc, delimiter=",")
    ...
    np.savetxt(os.path.join(data_path, f"{participant}_w20_Catch22_mean.csv"), mean_object_accuracy, delimiter=",")
...
if __name__ == "__main__":
    ...
    np.savetxt("/data/z5452142/experiment_object/Overal_w20_Catch22_mean.csv", overall_mean_object_accuracy, delimiter=",")
    np.savetxt("/data/z5452142/experiment_object/Overal_w20_Catch22_ci.csv", ci, delimiter=",")
    ...
    plt.tight_layout()
    plt.savefig("Overall_w20_Catch22.png")
    plt.show()
