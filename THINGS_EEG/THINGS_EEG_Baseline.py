...
def windowed_data_iteratively(data_object1, data_object2, time_bin, window_size):
    ...
    data1_windowed = data_object1[:, :, time_bin]
    data2_windowed = data_object2[:, :, time_bin]
...
def evaluate_over_time(data_object1, data_object2, time_bins, seq_numbers1, seq_numbers2):
    ...
    for t in time_bins:
        time_accuracies = []
        X_pca, y = windowed_data_iteratively(data_object1, data_object2, t, window_size = 0)
...
def process_participant(participant, base_dir="data/experiment_object"):
    ...
    for label in object_labels:
        ...
        selected_indices = indices[12:24]  # 12 trials per object
    ...
    for i in range(len(object_labels)):
        for j in range(i + 1, len(object_labels)):
            ...
            np.savetxt(os.path.join(data_path, f"{participant}_Baseline_{key}.csv"), acc, delimiter=",")
    ...
    np.savetxt(os.path.join(data_path, f"{participant}_Baseline_mean.csv"), mean_object_accuracy, delimiter=",")
...
if __name__ == "__main__":
    ...
    np.savetxt(os.path.join(results_dir, "Overall_Baseline_mean.csv"), overall_mean_object_accuracy, delimiter=",")
    np.savetxt(os.path.join(results_dir, "Overall_Baseline_ci.csv"), ci, delimiter=",")
