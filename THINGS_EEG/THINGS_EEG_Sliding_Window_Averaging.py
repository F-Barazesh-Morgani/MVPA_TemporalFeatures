...
def process_participant(participant, base_dir="data/experiment_object"):
    ...
    for i in range(len(object_labels)):
        for j in range(i + 1, len(object_labels)):
            ...
            np.savetxt(os.path.join(data_path, f"{participant}_w20_Sliding_Window_Averaging_{key}.csv"),
                       acc, delimiter=",")
    ...
    np.savetxt(os.path.join(data_path, f"{participant}_w20_Sliding_Window_Averaging_mean.csv"),
...
if __name__ == "__main__":
    ...
    np.savetxt(os.path.join(BASE_DIR, "Overall_w20_Sliding_Window_Averaging_mean.csv"),
               overall_mean_object_accuracy, delimiter=",")
    np.savetxt(os.path.join(BASE_DIR, "Overall_w20_Sliding_Window_Averaging_ci.csv"),
               ci, delimiter=",")
