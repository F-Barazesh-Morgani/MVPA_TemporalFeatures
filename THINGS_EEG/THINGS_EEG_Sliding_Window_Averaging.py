...
def evaluate_over_time(data_object1, data_object2, time_bins, seq_numbers1, seq_numbers2):
    ...
    for t in time_bins:
        time_accuracies = []
        X_scaled, y = windowed_data_iteratively(data_object1, data_object2, t, window_size = 20)
        
        for stim1 in np.unique(seq_numbers1):
            for stim2 in np.unique(seq_numbers2):
                if stim1 == stim2:  
                    X_train, Y_train, X_test, Y_test = train_test_split(X_scaled, y, seq_numbers1, seq_numbers2, stim1, stim2)
...
def process_participant(participant):
    ...
    for label in object_labels:
        ...
        # Keep only the second 12 instances
    ...
    for i in range(len(object_labels)):
        for j in range(i + 1, len(object_labels)):
            ...
            np.savetxt(os.path.join(data_path, f"{participant}_w20_Sliding_Window_Averaging_{key}.csv"), acc, delimiter=",")
    ...
    np.savetxt(os.path.join(data_path, f"{participant}_w20_Sliding_Window_Averaging_mean.csv"), mean_object_accuracy, delimiter=",")
...
if __name__ == "__main__":
    ...
    np.savetxt("/data/z5452142/experiment_object/Overal_w20_Sliding_Window_Averaging_mean.csv", overall_mean_object_accuracy, delimiter=",")
    np.savetxt("/data/z5452142/experiment_object/Overal_w20_Sliding_Window_Averaging_ci.csv", ci, delimiter=",")
    ...
    plt.xlabel('Time (ms)')
    plt.ylabel('Accuracy')
    plt.title('Average Classifier Accuracy Over Time - Sliding Window Averaging Method (w=20ms)')
