...
def process_participant(participant, base_dir="data/experiment_object"):
    ...
    for label in object_labels:
        ...
        sequence_numbers_per_object[label] = TrialList[selected_indices, 3]
    ...
    for i in range(len(object_labels)):
        for j in range(i + 1, len(object_labels)):
            ...
            acc = evaluate_over_time(object_data[object_a], object_data[object_b],
                                     time_bins, seq_numbers_object_a, seq_numbers_object_b)
            key = f"{object_a}v{object_b}"
            pairwise_accuracies[key] = acc

            np.savetxt(os.path.join(data_path, f"{participant}_w20_MiniRocket_{key}.csv"), acc, delimiter=",")
    ...
    np.savetxt(os.path.join(data_path, f"{participant}_w20_MiniRocket_mean.csv"), mean_object_accuracy, delimiter=",")
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        results = list(executor.map(lambda p: process_participant(p, base_dir=BASE_DIR), participants))
    ...
    np.savetxt(os.path.join(BASE_DIR, "Overall_w20_MiniRocket_mean.csv"), overall_mean_object_accuracy, delimiter=",")
    np.savetxt(os.path.join(BASE_DIR, "Overall_w20_MiniRocket_ci.csv"), ci, delimiter=",")
